...
- `schemas/input_schema.json` 约束混合词人人格 `lyricist_persona.ratio` 只能使用支持的比例格式。
- `schemas/output_schema.json` 约束 `fusion_notes.ratio` 的比例格式。
- `tests/test_test_runner.py` 增加对应回归测试。
- 同步更新 `README.md`、`README-GITHUB.md`、`test_runner.md` 的结构化校验说明；未手动追加 README 自动更新附录。
...
已完成一项小而完整的结构化合同补强，已由 GitHub autopilot 在验证后自动发布。

改动内容：
- `test_runner.py` 的内建 JSON Schema 子集新增 `uniqueItems` 校验。
- `schemas/input_schema.json` 对 `genre_tags`、`instruments`、`hard_constraints.ban_elements`、`hard_constraints.required_elements`、`fusion.focus_dimensions` 启用去重约束，避免重复输入参数被重复计权。
- `tests/test_test_runner.py` 增加重复数组参数的回归测试。
- 同步更新 `README.md`、`README-GITHUB.md`、`test_runner.md` 的结构化校验说明；没有手动追加 README 文末自动更新记录。
