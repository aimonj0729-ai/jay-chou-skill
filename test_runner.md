...
内建 JSON Schema 子集现在会检查 `type`、`enum`、`required`、`minItems` / `maxItems`、`minimum` / `maximum`、`oneOf`、`anyOf` 和 `additionalProperties`。两份结构化 schema 的顶层和主要嵌套对象已设置为不接受未声明字段，避免示例里出现拼写错误或旧字段时仍被判通过。
